...
export async function promptNewIssue() {
    ...
    return new Promise((resolve) => {
        ...
        top+=2;
        blessed.text({ parent: form, top: top, left: 1, content: 'Body:', style: { bg: THEME.paneBg, fg: THEME.paneFg } });
        const bodyIn = blessed.textarea({ parent: form, top: top, left: 15, width: '50%', height: 10, keys: true, mouse: true, inputOnFocus: true, style: { bg: THEME.inputBg, fg: THEME.inputFg } });
        
        top+=11;
        const submit = blessed.button({ parent: form, top: top, left: 1, shrink: true, padding: { left: 1, right: 1 }, content: 'Submit', style: { bg: THEME.buttonBg, fg: THEME.buttonFg, focus: { bg: THEME.buttonFocusBg } }, keys: true, mouse: true });
        
        submit.on('press', () => form.submit());
        form.on('submit', () => resolve({ title: titleIn.value, body: bodyIn.value }));
